...
\end{center}
	...
	%TODO Motivation why you should accept this paper or think it's important
	...
	%Ownership of the hash-space is assigned to be the space between a given node and it's closest predecessor.
	
	Traveling along the ring would give the network an unscalable  $O(n)$ lookup time for any record. 
	...
	%https://github.com/jech/dht <- reference implementation
	%http://bittorrent.org/beps/bep_0005.html <- c reference impl
	...
	%Because of the geometric awkwardness of it's metric, Kademlia uses a modified k-nearest neighbors approach to approximate a node's Voronoi regions and Delayney peers.
