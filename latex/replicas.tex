...
The half-life due to churn for the record (a time period over which, on average, the number of replicas in the network will be halved ) is $\frac{-\log(2)}{1-R_{c}}$ and because records are a small integer, we can also consider an expected mean lifetime of a record in terms of the number of replicas: $\frac{\log{K}}{\log(2)} * \frac{-\log(2)}{1-R_{c}}$
...
Failure in such a system is limited to overlay partition failures, where only one partition of the network is likely 
