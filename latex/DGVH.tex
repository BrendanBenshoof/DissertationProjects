...
Voronoi diagrams can be used as to manage distributed hash tables \cite{virtvoro}, or in coverage detection for wireless networks \cite{carbunar2004distributed}.
...
In addition, complications occur when points are located in spaces with more than two dimensions.
...
\begin{itemize}
	...
	In addition, DGVH can accommodate the calculation of nodes moving their positions and adjust their region accordingly, while still maintaining a high degree of accuracy (Section \ref{sec:dgvhdgvh}).
    Even where small inaccuracies exist, DVGH will create a fully connected graph.
	\item We discuss how P2P networks and distributed applications can use DGVH (Section \ref{sec:dgvhapplications}).
	...
	Our simulations show that a distributed network running DGVH accurately determines the region a randomly chosen point falls in 90\% of the time within 20 cycles and converges with near 100\% accuracy by cycle 30 (Section \ref{sec:dgvhexperiments}).
	\item We present the related work we have built upon to create our heuristic and what improvements we made with DGVH (Section \ref{sec:dgvhrelated}).
\end{itemize}
...
An example of a Voronoi diagram is shown in Figure \ref{voro-ex}.
...
The Distributed Greedy Voronoi Heuristic (DGVH) is a fast method for nodes a P2P network to define their individual Voronoi region (Figure \ref{alg:dgvh}).
If each node is assigned a location, DGVH builds an approximation of the nodes it is required to connect to such that it can accurately discern it's Voronoi region.
...
\[ distance = \sqrt{\sum\limits_{i\in d} (\min(| a_{i} -b_{i}|, 1-|a_{i}-b_{i}|))^2}\]
...
\begin{figure}
	...
	\label{occ-ex}
\end{figure}
Our algorithm handles these cases via our method of peer management (Section \ref{sec:dgvhmanage}).
...
The formal algorithm for this process is described in Figure~\ref{alg:gossip}.
...
\label{sec:dgvhapplications}

As we previously discussed in Section \ref{sec:dgvhintro}, Voronoi tessellations have many applications for distributed systems \cite{carbunar2004distributed} \cite{hu2004scalable} \cite{hu2008voronoi} \cite{Backhaus:2007:VAS:1326257.1326266}.
...
We test in 2-dimensional Euclidean space and measured the number of edges in the graph generated by DGVH that differ from the graph generated by a Delaunay Triangulation.
We test networks of 100, 500, 1000, 2000, and 5000 nodes and found that the graphs by DGVH differed from the graph generated Delaunay triangulation by approximately 1 edge per node.
...
In each cycle, the nodes gossip (Figure \ref{alg:gossip}) and run DGVH using the new information.
We then calculate the hit rate of successful lookups by simulating 2000 lookups from random nodes to random locations, as described in Figure \ref{routesim}.
...
As the network size and number of dimensions each increase, convergence slows.
